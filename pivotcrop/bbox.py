...
class BoundingBox:
    ...
    @staticmethod
    def compose(bbs: List[Optional["BoundingBox"]]) -> Optional["BoundingBox"]:
    ...
    def resize_with_pivot(self,  pivot: Pivot, total_bb: Optional["BoundingBox"] = None) -> "BoundingBox":
        """Given a bounding box that is contained within `total_bb`,
        Return `bb` shrunk as much as possible while keeping `pivot` at the same relative location.
        """
        if total_bb is None:
            total_bb = self
        adjusted_bb: BoundingBox = copy(self)
        ...
        if pivot[X] == 0.0:
            ...
            d_min_x: int = self.min_x - total_bb.min_x
            d_max_x: int = total_bb.max_x - self.max_x
        ...
        if pivot[Y] == 0.0:
            ...
            d_min_y: int = self.min_y - total_bb.min_y
            d_max_y: int = total_bb.max_y - self.max_y
